...
def up_song():
    ...
    print(a)
    _thread.start_new_thread(ciu, ("Hê lô", a))
...
def ciu(text, int):
    if int == "Các tăng giá trị bản thân gấp trăm lần":
        audio_file = os.path.dirname(__file__) + '/2.mp3'
    elif int == "Chuyện Hà Nội":
...
def get_text():
    for i in range(3):
        ...
        if text:
            ...
            speak("Không nhận diện được khẩu lệnh")
    return 0


def help_me():
    jarvis.speak("Bạn có thể nói lên, xuống để chọn sách")
    time.sleep(0.2)
    jarvis.speak("nói chơi hoặc bật để nghe sách")
    time.sleep(0.2)
    jarvis.speak("hoặc nói tạm dừng, tiếp tục để điều khiển")
    time.sleep(0.2)
...
def assistant_one(a, b):
    ...
    if "chào" in text:
        speak("chào")
    elif "bật" in text or "chơi" in text:
        play_song()
    elif "tắt" in text or "tạm dừng" in text:
        play_song()
    elif "giúp" in text or "hướng dẫn" in text:
        help_me()
    elif "lên" in text:
        up_song()
    elif "xuống" in text:
        down_song()
    elif "tua trước" in text:
        forward_song()
    elif "tua sau" in text:
        backward_song()
    elif "đánh dấu" in text:
        book_mark(1)
    elif "đánh dấu" in text:
        open_book_mark(1)
    else:
        speak("Không thể nhận diện")
